...
function ValidateForm(form) { 
 form = document.forms["frmProduct"];
	try{  console.log(" try..en-CA....");
		HideServerValidationBlock();
	}
	catch (everything){ console.log("catch......");
	}
	Validate(form);
	if (g_ErrMsg != '') {	 console.log("g_ErrMsg en-CA.......jsp  ."+g_ErrMsg);			
		var sWholeErrorMessage;
		sWholeErrorMessage = '<div class="ErrorHeader">';
		sWholeErrorMessage +=	'<img border="0" align="middle" alt="error" src="/o/ca.cityofkingston.payments' + sgBizImagesPath + '/yield.gif" />';
	...
	else {  console.log("else en-CA....jsp..");
		return true;
	}
}

function Validate(form) { console.log("validate form en-CA....jsp..........");
	var bAllowBlank;
	var bPassedRequired;
	var sStructure;
	var sAttribute;
	...
	g_ErrMsg = '';	
	for (var cntl=0;cntl<form.elements.length;cntl++) {  console.log("for loop en-CA:::::::::");
	console.log("form:::::::::"+form);
		strCurrentElementType = form.elements[cntl].type;
		console.log("strCurrentElementType:::::::::"+strCurrentElementType);
		g_ElementValue = form.elements[cntl].value;
		console.log("g_ElementValue::::::::"+g_ElementValue);
		g_ControlName = form.elements[cntl].getAttribute('caption');
		console.log("g_ControlName:::::::::"+g_ControlName);
		...
		console.log("sAttribute:::::::::"+sAttribute);
		
		if (sAttribute == 'yes' || sAttribute == 'true') { console.log("sAttribute en-CA:::::::::"+sAttribute);
			bAllowBlank = false;
			if (form.elements[cntl].type == 'checkbox') {
				if (form.elements[cntl].checked == false) {
					bPassedRequired = false;
					g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is required';
				}
			}
				...
				if (isNotBlank(g_ElementValue) == false){
					g_ElementValue = form.elements[cntl].options[intSelectedIndex].text;
				}
				if (isNotBlank(g_ElementValue) == false){
					bPassedRequired = false;
					g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is required';
		...
		/* else if(sAttribute == null) { console.log("else.....else....");
			bPassedRequired = false;
			g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is required';
			console.log("else.....g_ErrMsgg_ErrMsg...."+g_ErrMsg);
		} */
		else {   console.log("else....:::::::::");
			bAllowBlank = true;
			/* bPassedRequired = false;
			g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is required';
			console.log("else..else...g_ErrMsgg_ErrMsg...."+g_ErrMsg); */
		}
		console.log("for loop en-CA g_ErrMsg: ::::::::"+g_ErrMsg);
			...
			switch (sStructure) {
				case 'email': 
					if (IsEmail(g_ElementValue, bAllowBlank) == false) {
						g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid email';
					}
					break;
				...
				case 'number':
					...
					if (IsNumber(g_ElementValue,bAllowBlank,bAllowDecimal,bAllowNegative) == false) {
						g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid number';
					}
					else {
					...
					case 'date':
						...
						if (IsDate(oDate, bAllowBlank, sAttribute) == false) {
							switch(sAttribute.toLowerCase()) {
								case 'y/m/d':
									g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid date. Use format yyyy/mm/dd';
									break;
									
								case 'y/d/m': 
									g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid date. Use format yyyy/dd/mm';
									break;
									
								case 'm/d/y': 
									g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid date. Use format mm/dd/yyyy';
									break;
									
								case 'd/m/y': 
									g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid date. Use format dd/mm/yyyy';
									break;
							}
						}
						else {
					...
					case 'postalcode':
						if (IsPostalCode(g_ElementValue, bAllowBlank) == false) {
							g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid Post Code';
						}
						break;

					case 'zipcode':
						if (IsZipCode(g_ElementValue, bAllowBlank) == false) {
							g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid Zip Code';
						}
						break;

					case 'ziporpostalcode':
						if (IsZipOrPostalCode(g_ElementValue, bAllowBlank) == false) {
							g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid Zip or Postal Code';
						}
						break;
					...
					case 'money': 
						...
						if (IsMoney(g_ElementValue,bAllowBlank,bAllowNegative) == false) {
							g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid currency amount';
						}
						else {
	...
	if (g_ErrMsg == '') { console.log("msg empty en-cA .........")
		var sFieldToValidate;
		var sFieldToValidateWith;
		var sOperation;
		var sCaption;
...
function IsTelephone(fieldVal, bAllowNull) {
	...
	if (pocMASK.test(fieldVal) != true) {
		g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not a valid telephone number';
		return false;
	}
	else {
		return true;
	...
	if (IsNumber(Expression, false, true, true)) {
		...
		if (IsNumber(MinValue,false, true, true) && IsNumber(MaxValue,false, true, true)) {
			if ((parseFloat(Expression) >= parseFloat(MinValue) == false) || (parseFloat(Expression) <= parseFloat(MaxValue) == false)) {
				sErrMsg = ' is not valid. The number must be between <MinValue> and <MaxValue>';
		...
		if (IsNumber(MaxValue, false, true, true)) {
			if ((parseFloat(Expression) <= parseFloat(MaxValue)) == false) {
				sErrMsg = ' is not valid. The number must be less than or equal to <MaxValue>';
				g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' + Replace(sErrMsg, '<MaxValue>', MaxValue);
				return false;
			}
			else {
		...
		if (IsNumber(MinValue, false, true, true)) {
			if ((parseFloat(Expression) >= parseFloat(MinValue)) == false) {
				sErrMsg = ' is not valid. Must be a number greater than or equal to <MinValue>';
...
function IsZipOrPostalCode(field, bAllowBlank) {
	...
	if (matchArray == null) {
		g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not in a valid time. Use the HH:MM or HH:MM:SS format';
		return false;
	}
	hour = matchArray[1];
	minute = matchArray[3];
	...
	if (hour < 0|| hour > 23) {
		g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not in a valid time. Hour must be between 1 and 23.';
		return false;
	}		
	if (minute<0 || minute > 59) {
		g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' Minute must be between 0 and 59.';
		return false;
	}
	if (second != null && (second < 0 || second > 59)) {
		g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' is not in a valid time. Second must be between 0 and 59.';
		return false;
	}
	if (second==null) {
		fieldVal += ':00';
...
function IsWithinDateRange(Expression,MinValue,MaxValue,bAllowBlank,sFormat) {
	...
	if (IsDate(oMinDate) && IsDate(oMaxDate)) {
		if ((Expression >= oMinDate.datevalue) == false || (Expression <= oMaxDate.datevalue) == false) {
			sErrMsg = 'must be between the following dates: <MinValue> and <MaxValue>';
	...
	if (IsDate(oMaxDate)) {
		if ((Expression <= oMaxDate.datevalue) == false) {
			sErrMsg = 'cannot occur in the future';
			g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' +Replace(sErrMsg, '<MaxValue>', FormatDate(oMaxDate.datevalue,sFormat));
			return false;
		}
		else {
	...
	if (IsDate(oMinDate)){
		if ((Expression >= oMinDate.datevalue) == false) {
			sErrMsg = 'cannot occur in the past';
...
function IsWithinTextRange(Expression,MinValue,MaxValue,bAllowBlank) {
	...
	if (IsNumber(MinValue,false, false, false) && IsNumber(MaxValue,false, false, false)) {
		if ((Expression.length >= MinValue) == false || (Expression.length <= MaxValue) == false) {
			if (MinValue == MaxValue) {
				sErrMsg = 'must be <MinValue> characters in length';
				g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' + Replace(sErrMsg, '<MinValue>', MinValue);
			}
			else {
				sErrMsg = 'must be between <MinValue> and <MaxValue> characters in length';
				sErrMsg = Replace(sErrMsg, '<MinValue>', MinValue);
				g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' + Replace(sErrMsg, '<MaxValue>', MaxValue);
			}
	...
	if (IsNumber(MaxValue, false, false, false)) {
		if ((Expression.length <= MaxValue) == false) {
			sErrMsg = 'must be less than or equal to <MaxValue> characters in length';
			g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' + Replace(sErrMsg, '<MaxValue>', MaxValue);
			return false;
		}
		else {
	...
	if (IsNumber(MinValue, false, false, false)) {
		if ((Expression.length >= MinValue) == false) {
			sErrMsg = 'must be greater than or equal to <MinValue> characters in length';
			g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' + Replace(sErrMsg, '<MinValue>', MinValue);
			return false;
		}
		else {
...
function IsRegEx(sInputExpression, sDefinedExpression, bAllowBlank) {
	if (sInputExpression == null) 
		return (bAllowBlank);
	if (sInputExpression.length == 0) 
		return (bAllowBlank);
	var oRegExp;
