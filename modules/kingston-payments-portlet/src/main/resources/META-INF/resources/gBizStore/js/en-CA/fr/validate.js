 //The following are unicode used in the text:
 // \u00E9 = é
 // \u00E8 = è
 // \u00EA = ê
 // \u00E0 = à
 ...
 // \u0027 = '
	...
	function ValidateForm(form) {
		...
		if (g_ErrMsg != '') {			
			...
			sWholeErrorMessage +=	'Veuillez corriger les erreurs suivantes:';
			...
			if (sAttribute == 'yes' || sAttribute == 'true') {
				...
				if (form.elements[cntl].type == 'checkbox') {
					if (form.elements[cntl].checked == false) {
						...
						g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' est exig\u00E9 ' + g_FixStr;
					}
				}
				...
				switch (sStructure) {
					case 'email': 
						if (IsEmail(g_ElementValue, bAllowBlank) == false) {
							g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' est pas un email valide ' + g_FixStr;
						}
						break;
					...
					case 'number':
						...
						if (IsNumber(g_ElementValue,bAllowBlank,bAllowDecimal,bAllowNegative) == false) {
							g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' n\u0027est pas un nombre valide ' + g_FixStr;
						}
						else {
						...
						case 'date':
							...
							if (IsDate(oDate, bAllowBlank, sAttribute) == false) {
								switch(sAttribute.toLowerCase()) {
									case 'y/m/d':
										g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' n\u0027est pas une date valide.  Employez le format aaaa/mm/jj ' + g_FixStr;
										break;
										
									case 'y/d/m': 
										g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' n\u0027est pas une date valide.  Employez le format aaaa/jj/mm ' + g_FixStr;
										break;
										
									case 'm/d/y': 
										g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' n\u0027est pas une date valide.  Employez le format mm/jj/aaaa ' + g_FixStr;
										break;
										
									case 'd/m/y': 
										g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' n\u0027est pas une date valide.  Employez le format jj/mm/aaaa ' + g_FixStr;
										break;
								}
							}
							else {
						...
						case 'postalcode':
							if (IsPostalCode(g_ElementValue, bAllowBlank) == false) {
								g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' n\u0027est pas un code postal valide ' + g_FixStr;
							}
							break;

						case 'zipcode':
							if (IsZipCode(g_ElementValue, bAllowBlank) == false) {
								g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + '  n\u0027est pas un code zip valide ' + g_FixStr;
							}
							break;

						case 'ziporpostalcode':
							if (IsZipOrPostalCode(g_ElementValue, bAllowBlank) == false) {
								g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + '  n\u0027est pas un code zip ou postal valide ' + g_FixStr;
							}
							break;
						...
						case 'money': 
							...
							if (IsMoney(g_ElementValue,bAllowBlank,bAllowNegative) == false) {
								g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' n\u0027est pas une quantit\u00E9 mon\u00E9taire valide ' + g_FixStr;
							}
							else {
...
function RequiredGroupValidator(sOperation,sFieldsToValidate,sDefaultErrorMsg) {	 
		...
		switch(sFormat.toLowerCase()) {
			...
			case 'dd-mmm-yyyy': 
				switch(sDate.substr(5, 2)) {
					case '01': 
						return (sDate.substr(8,2) + '-Janv.-' + sDate.substr(0,4));
					case '02': 
						return (sDate.substr(8,2) + '-F\u00E9v.-' + sDate.substr(0,4));
					case '03': 
						return (sDate.substr(8,2) + '-Mars-' + sDate.substr(0,4));
					case '04': 
						return (sDate.substr(8,2) + '-Avr.-' + sDate.substr(0,4));
					case '05': 
						return (sDate.substr(8,2) + '-Mai-' + sDate.substr(0,4));
					case '06': 
						return (sDate.substr(8,2) + '-Juin-' + sDate.substr(0,4));
					case '07': 
						return (sDate.substr(8,2) + '-Juillet-' + sDate.substr(0,4));
					case '08': 
						return (sDate.substr(8,2) + '-Ao\u00FCt-' + sDate.substr(0,4));
					case '09': 
					...
					case '12': 
						return (sDate.substr(8,2) + '-D\u00E9c.-' + sDate.substr(0,4));
					}
			default: 
				return (sDate.substr(8,2) + '//' + sDate.substr(5,2) + '//' + sDate.substr(0,4));
		}
	...
	function IsTelephone(fieldVal, bAllowNull) {
		...
		if (pocMASK.test(fieldVal) != true) {
			g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' n\u0027est pas un num\u00E9ro de t\u00E9l\u00E9phone valide ' + g_FixStr;
			return false;
		}
		else {
			return true;
		...
		if (IsNumber(Expression, false, true, true)) {
			...
			if (IsNumber(MinValue,false, true, true) && IsNumber(MaxValue,false, true, true)) {
				if ((parseFloat(Expression) >= parseFloat(MinValue) == false) || (parseFloat(Expression) <= parseFloat(MaxValue) == false)) {
					sErrMsg = ' est inadmissible.  Le nombre doit \u00EAtre entre <MinValue> et <MaxValue> ' + g_FixStr;
			...
			if (IsNumber(MaxValue, false, true, true)) {
				if ((parseFloat(Expression) <= parseFloat(MaxValue)) == false) {
					sErrMsg = ' est inadmissible.  Le nombre doit \u00EAtre inf\u00E9rieur ou \u00E9gal \u00E0 <MaxValue> ' + g_FixStr;
					g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' + Replace(sErrMsg, '<MaxValue>', MaxValue);
					return false;
				}
				else {
			...
			if (IsNumber(MinValue, false, true, true)) {
				if ((parseFloat(Expression) >= parseFloat(MinValue)) == false) {
					sErrMsg = ' est inadmissible.  Doit \u00EAtre un nombre sup\u00E9rieur ou \u00E9gal \u00E0 <MinValue> ' + g_FixStr;
	...
	function IsZipOrPostalCode(field, bAllowBlank) {
 	...
 	function IsTime(fieldVal, bAllowBlank) {
		...
		if (matchArray == null) {
			g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' n\u0027est pas un temps valide.  Employez le format HH:MM ou HH:MM:SS ' + g_FixStr;
			return false;
		}
		hour = matchArray[1];
		minute = matchArray[3];
		...
		if (hour < 0|| hour > 23) {
			g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + '  n\u0027est pas un temps valide. L\u0027heure doit \u00EAtre entre 1 et 23. ' + g_FixStr;
			return false;
		}		
		if (minute<0 || minute > 59) {
			g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' les minutes doit \u00EAtre entre 0 et 59. ' + g_FixStr;
			return false;
		}
		if (second != null && (second < 0 || second > 59)) {
			g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + '  n\u0027est pas un temps valide. Les secondes doivent \u00EAtre entre 0 et 59. ' + g_FixStr;
			return false;
		}
		if (second==null) {
			fieldVal += ':00';
	...
	function IsWithinDateRange(Expression,MinValue,MaxValue,bAllowBlank,sFormat) {
		...
		if (IsDate(oMinDate) && IsDate(oMaxDate)) {
			if ((Expression >= oMinDate.datevalue) == false || (Expression <= oMaxDate.datevalue) == false) {
				sErrMsg = 'doit avoir lieu entre les dates suivantes:  <MinValue> et <MaxValue> ' + g_FixStr;
		...
		if (IsDate(oMaxDate)) {
			if ((Expression <= oMaxDate.datevalue) == false) {
				sErrMsg = 'doit \u00EAtre inf\u00E9rieur ou \u00E9gal \u00E0 la date suivante: <MaxValue> ' + g_FixStr;
				g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' +Replace(sErrMsg, '<MaxValue>', FormatDate(oMaxDate.datevalue,sFormat));
				return false;
			}
			else {
		...
		if (IsDate(oMinDate)){
			if ((Expression >= oMinDate.datevalue) == false) {
				sErrMsg = 'doit \u00EAtre sup\u00E9rieur ou \u00E9gal \u00E0 la date suivante: <MinValue> ' + g_FixStr;
	...
	function IsWithinTextRange(Expression,MinValue,MaxValue,bAllowBlank) {
		...
		if (IsNumber(MinValue,false, false, false) && IsNumber(MaxValue,false, false, false)) {
			if ((Expression.length >= MinValue) == false || (Expression.length <= MaxValue) == false) {
				if (MinValue == MaxValue) {
					sErrMsg = 'doivent \u00EAtre <MinValue> caract\u00E8res de long ' + g_FixStr;
					g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' + Replace(sErrMsg, '<MinValue>', MinValue);
				}
				else {
					sErrMsg = 'doit \u00EAtre entre <MinValue> et <MaxValue> caract\u00E8res de long ' + g_FixStr;
					sErrMsg = Replace(sErrMsg, '<MinValue>', MinValue);
					g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' + Replace(sErrMsg, '<MaxValue>', MaxValue);
				}
		...
		if (IsNumber(MaxValue, false, false, false)) {
			if ((Expression.length <= MaxValue) == false) {
				sErrMsg = 'doit  \u00EAtre inf\u00E9rieur ou \u00E9gal \u00E0 <MaxValue> caract\u00E8res de long ' + g_FixStr;
				g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' + Replace(sErrMsg, '<MaxValue>', MaxValue);
				return false;
			}
			else {
		...
		if (IsNumber(MinValue, false, false, false)) {
			if ((Expression.length >= MinValue) == false) {
				sErrMsg = 'doit \u00EAtre sup\u00E9rieur ou \u00E9gal \u00E0 <MinValue> caract\u00E8res de long ' + g_FixStr;
				g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + g_ControlName + ' ' + Replace(sErrMsg, '<MinValue>', MinValue);
				return false;
			}
			else {
			...
			case 'gteq': 
				if (dValValue < dValWithValue) {
					if (sDefaultErrorMsg.length > 0) {
						...
						g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + sFieldName + ' doit \u00EAtre sup\u00E9rieur ou \u00E9gal \u00E0 ' + sFieldWithName;
					}
					return false;
				}
				break;
			...
			case 'gt': 
				if (dValValue <= dValWithValue) {
					if (sDefaultErrorMsg.length > 0) {
						...
						g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + sFieldName + ' doit \u00EAtre plus grand que ' + sFieldWithName;
					}
					return false;
				}
				break;
			...
			case 'lteq': 
				if (dValValue > dValWithValue) {
					if (sDefaultErrorMsg.length > 0) {
						...
						g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + sFieldName + ' doit \u00EAtre inf\u00E9rieur ou \u00E9gal \u00E0 ' + sFieldWithName;
					}
					return false;
				}
				break;
			...
			case 'lt': 
				if (dValValue >= dValWithValue) {
					if (sDefaultErrorMsg.length > 0) {
						...
						g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + sFieldName + ' doit \u00EAtre moins que ' + sFieldWithName;
					}
					return false;
				}
				break;
			...
			case 'eq': 
				if (dValValue != dValWithValue) {
					if (sDefaultErrorMsg.length > 0) {
						...
						g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + sFieldName + ' doit \u00EAtre \u00E9gal \u00E0 ' + sFieldWithName;
					}
					return false;
				}
				break;
		...
		if (sOperation.toLowerCase() == 'or') {
			if (sValValue.length == 0 && sValWithValue.length == 0) {
				if (sDefaultErrorMsg == null) {
					g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + 'L\u0027un ou l\u0027autre (' + sFieldName + ' ou ' + sFieldWithName + ') est exig\u00E9';
				}
				else {
					g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + sDefaultErrorMsg;
				}
			...
			if (sValValue.length == 0 && sValWithValue.length > 0) {
				if (sDefaultErrorMsg == null) {
					g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + sFieldName + ' est exig\u00E9 si une valeur pour ' + sFieldWithName + ' est fourni';
				}
				else {
					g_ErrMsg = g_ErrMsg + '<li class="ErrorDetail">' + sDefaultErrorMsg;
				}
